...
" Description: cus a visual selection with Neural.

let s:current_job = get(s:, 'current_cus_job', 0)
...
function! neural#cus#Cleanup() abort
    if s:current_job
...
function! neural#cus#SelectedLines(prompt) abort
    " Reload the Neural config if needed.
    call neural#config#Load()
    " Stop Neural doing anything else if cusing code.
    call neural#Cleanup()
